...
; Loop body exceeds MaxAsyncMarkers(=32) on first iteration
; Preloop: 5 markers
; Loop body: 35 markers
...
entry:
  ...
  call void @llvm.amdgcn.global.load.lds(ptr addrspace(1) %in, ptr addrspace(3) %lds, i32 4, i32 0, i32 u0x20)
...
loop_body:
  ; Loop body with 35 async operations
...
; Loop body does not exceed MaxAsyncMarkers(=32) on first iteration
; Preloop: 5 markers
; Loop body: 5 markers
...
; Merge exceeds MaxAsyncMarkers(=32)

; CHECK-LABEL: max_when_merged:
; CHECK: ; wait_asyncmark(33)
...
then:
  ...
  call void @llvm.amdgcn.wait.asyncmark(i16 33)
  %lds_val = load i32, ptr addrspace(3) %lds
  store i32 %lds_val, ptr addrspace(1) %out
  ret void
}

; Straightline exceeds MaxAsyncMarkers(=32)
